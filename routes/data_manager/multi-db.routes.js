...
module.exports = [
            ...
            for (const id of intIds) {
              attributes.forEach((attr) => {
                ...
                result.push({
                  ...
                  value:  typeof v === 'object' ? ( v?.toLocaleString ? v.toLocaleString() : JSON.stringify(v) ) : v //$atom(value),
                });
              });
            }
          }
  ...
  {
    route: `dama[{keys:pgEnvs}].viewsbyId[{keys:viewIds}].options[{keys:options}].length`,
    get: async function(pathSet) {
      ...
      //                           })
      try {
        const { pgEnvs, viewIds, options } = pathSet;
        const result = [];

        for (const pgEnv of pgEnvs) {
          for (const viewId of viewIds) {
            for (const option of options) {
              const numRows = await simpleFilterLength(pgEnv, viewId, option);

              result.push({
                path: ["dama", pgEnv, "viewsbyId", viewId, "options", option, "length"],
                value: +numRows,
              });
            }
          }
...

  {
    ...
    route: `dama[{keys:pgEnvs}].viewsbyId[{keys:viewIds}].options[{keys:options}].databyIndex[{integers:indices}][{keys:attributes}]`,
    get: async function(pathSet) {
      try {
        // options: JSON.stringify({
        //                           filter: { geoid: [36001, ...], ...},
        //                           exclude: { year: [2011, 2012, ..], ... },
        //                           groupBy: [geoid, ...],
        //                           having: ['clause1', 'clause2', ...],
        //                           orderBy: [geoid, ...]
        //                           })
        // attributes: [geoid, namelsad as name, substring(geoid, 1, 2) as state_fips, ...]

        const { pgEnvs, viewIds, options, indices, attributes  } = pathSet;
        const result = [];
            ...
            for (const option of options) {
              ...
              indices.forEach(i => {
                attributes.forEach(attribute => {
                  ...
                  result.push({
                    path: ["dama", pgEnv, "viewsbyId", viewId, "options", option, "databyIndex", i, attribute],
                    value: typeof _.get(rows, [i, modifiedName], null) === 'object' ? $atom(_.get(rows, [i, modifiedName], null)) : _.get(rows, [i, modifiedName])
                  });
                })
              })
