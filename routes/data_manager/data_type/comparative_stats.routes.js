...
module.exports = [
  ...
  {
    route: `comparative_stats[{keys:pgEnvs}].byEalIds.source[{keys:sourceIds}].view[{keys:viewIds}].byGeoid`,
    get: async function(pathSet) {
      try {
        const { pgEnvs, sourceIds, viewIds } = pathSet;
        const result = [];
        ...
        for (const pgEnv of pgEnvs) {
          for(const sourceId of sourceIds){
            for (const viewId of viewIds) {
              const rows = await EALByGeoidByNriCat(pgEnv, sourceId, viewId);
              result.push({
                path: ['comparative_stats', pgEnv, 'byEalIds', 'source', sourceId, 'view', viewId, 'byGeoid'],
                value: $atom(rows)
              });
            }
          }
